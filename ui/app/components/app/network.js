  ...
  return (
    <div
      className={classnames('network-component pointer', {
        ...
        'xdc-network': providerName === 'xdc',
        'xdc-apothem-network': providerName === 'xdcApothem',
      })}
      title={hoverText}
      onClick={(event) => {
        if (!disabled) {
...
export default class Network extends Component {
  ...
  render () {
    ...
    switch (providerName) {
      ...
      case 'xdc':
        return (
          <NetworkIndicator disabled={disabled} hoverText={t('xdc')} onClick={onClick} providerName={providerName}>
            <NetworkDropdownIcon
              backgroundColor="#3099f2"
              nonSelectBackgroundColor="#ecb23e"
              loading={networkNumber === 'loading'}
            />
            <div className="network-name">{t('xdc')}</div>
          </NetworkIndicator>
        )
        case 'xdcApothem':
          return (
            <NetworkIndicator disabled={disabled} hoverText={t('xdcApothem')} onClick={onClick} providerName={providerName}>
              <NetworkDropdownIcon
                backgroundColor="#3099f2"
                nonSelectBackgroundColor="#ecb23e"
                loading={networkNumber === 'loading'}
              />
              <div className="network-name">{t('xdcApothem')}</div>
            </NetworkIndicator>
          )
