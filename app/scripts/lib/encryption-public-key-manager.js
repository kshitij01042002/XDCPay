...
export default class EncryptionPublicKeyManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync (address, req) {
    return new Promise((resolve, reject) => {
      if (!address) {
        reject(new Error('XDCPay Message: address field is required.'))
        return
      }
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          case 'received':
            ...
            return
          case 'rejected':
            reject(ethErrors.provider.userRejectedRequest('XDCPay EncryptionPublicKey: User denied message EncryptionPublicKey.'))
            return
          default:
            reject(new Error(`XDCPay EncryptionPublicKey: Unknown problem: ${JSON.stringify(address)}`))
        }
      })
    })
  }
